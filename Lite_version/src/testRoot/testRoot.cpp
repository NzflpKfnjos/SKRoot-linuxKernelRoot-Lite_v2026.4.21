...
void print_kpm_report(const kernel_root::KpmTestReport& report) {
	printf("%s", report.output.c_str());
	printf("kpm result:%s\n", to_string(report.err).c_str());
}

void test_kpm_info(int argc, char* argv[]) {
	if (argc < 1) {
		printf("Usage: testRoot kpmInfo <kpm-path>\n");
		return;
	}
	kernel_root::KpmElfInfo info;
	KRootErr err = kernel_root::parse_kpm_elf_info(argv[0], info);
	printf("%s", kernel_root::format_kpm_elf_info(info).c_str());
	printf("kpmInfo err:%s\n", to_string(err).c_str());
}

void test_kpm_check(int argc, char* argv[]) {
	if (argc < 1) {
		printf("Usage: testRoot kpmCheck <kpm-path>\n");
		return;
	}
	kernel_root::KpmTestReport report;
	kernel_root::kpm_compat_precheck(ROOT_KEY, argv[0], report);
	print_kpm_report(report);
}

void test_kpm_load_temp(int argc, char* argv[]) {
	if (argc < 1) {
		printf("Usage: testRoot kpmLoadTemp <kpm-path>\n");
		return;
	}
	kernel_root::KpmTestReport report;
	kernel_root::kpm_load_temp(ROOT_KEY, argv[0], report);
	print_kpm_report(report);
}

void test_kpm_unload(int argc, char* argv[]) {
	if (argc < 1) {
		printf("Usage: testRoot kpmUnload <module-name>\n");
		return;
	}
	kernel_root::KpmTestReport report;
	kernel_root::kpm_unload_by_name(ROOT_KEY, argv[0], report);
	print_kpm_report(report);
}
...
int main(int argc, char* argv[]) {
	printf(
		...
		"9. KPM静态解析\n"
		"\tUsage: testRoot kpmInfo <kpm-path>\n\n"

		"10. KPM兼容性预检\n"
		"\tUsage: testRoot kpmCheck <kpm-path>\n\n"

		"11. KPM临时加载测试\n"
		"\tUsage: testRoot kpmLoadTemp <kpm-path>\n\n"

		"12. KPM卸载\n"
		"\tUsage: testRoot kpmUnload <module-name>\n\n"
	...
	std::map<std::string, std::function<void()>> command_map = {
		...
		{"kpmInfo", [argc, argv]() { test_kpm_info(argc - 1, argv + 1); }},
		{"kpmCheck", [argc, argv]() { test_kpm_check(argc - 1, argv + 1); }},
		{"kpmLoadTemp", [argc, argv]() { test_kpm_load_temp(argc - 1, argv + 1); }},
		{"kpmUnload", [argc, argv]() { test_kpm_unload(argc - 1, argv + 1); }}
	};
